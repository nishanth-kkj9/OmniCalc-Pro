    ...
    <div className="max-w-4xl mx-auto w-full p-4 flex flex-col gap-6">
      ...
      {activeTab === 'emi' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-5">
            <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">Loan Inputs</h3>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Annual Interest Rate (%)</label>
              <input
                type="number"
                step="0.1"
                value={rate}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col justify-between gap-4">
            <div>
              <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">Monthly Payment Summary</h3>
              <div className="mt-4 flex flex-col gap-3">
                <div className="bg-sky-950/60 border border-sky-800/60 rounded-2xl p-4">
                  ...
                  <div className="text-3xl font-mono font-bold text-sky-300">${emiRes.emi.toFixed(2)}</div>
                </div>

                <div className="grid grid-cols-2 gap-3 text-xs font-mono">
                  <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
                    <span className="text-slate-400 block">Total Interest</span>
                    <span className="text-rose-400 font-bold text-base">${emiRes.totalInterest.toFixed(2)}</span>
                  </div>
                  <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
                    <span className="text-slate-400 block">Total Payable</span>
                    <span className="text-slate-100 font-bold text-base">${emiRes.totalPayable.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="h-48 w-full mt-2">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieDataEmi} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={40} outerRadius={65} paddingAngle={4}>
      ...
      {activeTab === 'compound' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-4">
            <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">Compound Investment</h3>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Initial Principal ($)</label>
              <input type="number" value={ciPrincipal} onChange={(e) => setCiPrincipal(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Annual Return Rate (%)</label>
              <input type="number" step="0.1" value={ciRate} onChange={(e) => setCiRate(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Duration (Years)</label>
              <input type="number" value={ciYears} onChange={(e) => setCiYears(parseInt(e.target.value) || 1)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Compounding Frequency</label>
              <select value={ciFreq} onChange={(e) => setCiFreq(parseInt(e.target.value))} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-sm font-semibold text-slate-200">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col justify-center gap-6">
            <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">Future Growth Projection</h3>
            <div className="bg-emerald-950/60 border border-emerald-800/60 rounded-2xl p-5">
              ...
              <div className="text-4xl font-mono font-bold text-emerald-300 mt-1">${ciRes.amount.toFixed(2)}</div>
            </div>
            <div className="grid grid-cols-2 gap-4 font-mono text-sm">
              <div className="bg-slate-800/60 p-4 rounded-2xl border border-slate-700/60">
                <span className="text-slate-400 text-xs block">Total Interest Earned</span>
                <span className="text-emerald-400 font-bold text-xl">${ciRes.interest.toFixed(2)}</span>
              </div>
              <div className="bg-slate-800/60 p-4 rounded-2xl border border-slate-700/60">
      ...
      {activeTab === 'gst' && (
        <div className="max-w-xl mx-auto w-full bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-5">
          <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">GST & Tax Calculation</h3>
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold text-slate-400">Amount ($)</label>
            <input type="number" value={gstAmount} onChange={(e) => setGstAmount(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold text-slate-400">Tax Rate (%)</label>
            <input type="number" value={gstRate} onChange={(e) => setGstRate(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsInclusive(false)} className={`flex-1 py-2.5 rounded-xl font-bold text-xs border transition-all ${!isInclusive ? 'bg-sky-600 text-white border-sky-500' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
              GST Exclusive (Add Tax)
            </button>
            <button onClick={() => setIsInclusive(true)} className={`flex-1 py-2.5 rounded-xl font-bold text-xs border transition-all ${isInclusive ? 'bg-sky-600 text-white border-sky-500' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
          ...
          <div className="border-t border-slate-800/80 pt-4 grid grid-cols-3 gap-3 font-mono text-center">
            <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
              ...
              <span className="text-slate-100 font-bold text-base">${gstRes.original.toFixed(2)}</span>
            </div>
            <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
      ...
      {activeTab === 'discount' && (
        <div className="max-w-xl mx-auto w-full bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-5">
          <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">Discount & Savings</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Original Price ($)</label>
              <input type="number" value={discPrice} onChange={(e) => setDiscPrice(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Discount (%)</label>
              <input type="number" value={discPct} onChange={(e) => setDiscPct(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 font-mono text-center pt-2">
            <div className="bg-emerald-950/60 border border-emerald-800/60 p-4 rounded-2xl">
              <span className="text-xs text-emerald-400 font-bold uppercase block">Amount Saved</span>
      ...
      {activeTab === 'roi' && (
        <div className="max-w-xl mx-auto w-full bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-5">
          <h3 className="text-sm font-bold text-slate-200 border-b border-slate-800 pb-3">Return on Investment (ROI)</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Initial Investment ($)</label>
              <input type="number" value={roiInitial} onChange={(e) => setRoiInitial(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-slate-400">Final Return Value ($)</label>
              <input type="number" value={roiFinal} onChange={(e) => setRoiFinal(parseFloat(e.target.value) || 0)} className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-lg font-mono font-bold text-slate-100" />
            </div>
          </div>
