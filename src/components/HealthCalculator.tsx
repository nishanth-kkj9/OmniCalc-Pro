...
export const HealthCalculator: React.FC<HealthCalculatorProps> = ({ settings: _settings }) => {
  ...
  const calculateHeartRate = () => {
    ...
    const zones = [
      ...
      {
        name: 'Endurance / Cardio (70-80%)',
        min: restingHr + hrr * 0.7,
        max: restingHr + hrr * 0.8,
      },
  ...
  return (
    <div className="max-w-4xl mx-auto w-full p-4 flex flex-col gap-6">
      ...
      <div
        ...
        onKeyDown={(e) =>
          handleTablistKeydown(e, [...HEALTH_TABS], tab, setTab as (tab: string) => void)
        }
      >
        {[
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col gap-6">
        ...
        <div
          role="tabpanel"
          id="health-panel-bmi"
          aria-labelledby="health-tab-bmi"
          hidden={tab !== 'bmi'}
        ...
        <div
          role="tabpanel"
          id="health-panel-bmr_tdee"
          aria-labelledby="health-tab-bmr_tdee"
          hidden={tab !== 'bmr_tdee'}
          className="flex flex-col gap-6"
        >
          <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 bg-slate-950 border border-slate-800 rounded-2xl p-5">
            <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
              ...
              <span className="text-xl font-mono font-bold text-slate-100">{bmrRes.bmr} kcal</span>
            </div>
            <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
              <span className="text-xs text-slate-400 block">TDEE (Maintenance)</span>
              <span className="text-xl font-mono font-bold text-sky-400">{bmrRes.tdee} kcal</span>
            </div>
            <div className="p-3 bg-slate-900 border border-slate-800 rounded-xl">
        ...
        <div
          role="tabpanel"
          id="health-panel-heart_rate"
          aria-labelledby="health-tab-heart_rate"
          hidden={tab !== 'heart_rate'}
          className="flex flex-col gap-6"
        >
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-slate-400">Resting Heart Rate (BPM)</label>
              <input
                type="number"
                value={restingHr}
